...
function buildCoupon(fixtures) {
  ...
  // VIKTIG: kombinert sannsynlighet regnes som produktet av beina, noe som KUN
  // er riktig hvis beina er uavhengige - det er de bare på tvers av ULIKE kamper.
  // To bein fra SAMME kamp (f.eks. "Arsenal vinner" og "Over 3,5 mål") henger
  // åpenbart sammen, og skal derfor aldri telles som to uavhengige bein - maks
  // ett bein pr kamp her (se comboMarkets i lib/poisson.js hvis man faktisk vil
  // kombinere flere markeder i SAMME kamp riktig, med reell fellessannsynlighet).
  const pool = spicy.filter((x) => x.p > 0.20 && x.p < 0.62).sort((a, b) => b.p - a.p);
  let legs = [], combP = 1, combO = 1;
  const usedMatches = new Set();
  for (const leg of pool) {
    if (usedMatches.has(leg.match)) continue;
    legs.push(leg); combP *= leg.p; combO /= leg.p; usedMatches.add(leg.match);
    if (combO >= 10) break;
  }
  if (combO < 10) {
    for (const leg of [...spicy].sort((a, b) => a.p - b.p)) {
      if (legs.includes(leg) || usedMatches.has(leg.match)) continue;
      legs.push(leg); combP *= leg.p; combO /= leg.p; usedMatches.add(leg.match);
      if (combO >= 10) break;
    }
  }
